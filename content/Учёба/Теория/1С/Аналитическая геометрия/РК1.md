...
### 2. Связь с общим уравнением

Любое общее уравнение прямой Ax + By + C = 0 можно привести к нормальному виду.

Сравнивая Ax + By + C = 0 и (cosα)x + (sinα)y - p = 0, видим, что коэффициенты должны быть пропорциональны:
*   $A = k \cdot \cos\alpha$
*   $B = k \cdot \sin\alpha$
*   $C = -k \cdot p$

Возведем первые два равенства в квадрат и сложим:
$A^2 + B^2 = k^2(\cos^2\alpha + \sin^2\alpha) = k^2 \cdot 1 \implies k = \pm\sqrt{A^2+B^2}$.

Этот k — **нормирующий множитель**. Чтобы привести общее уравнение к нормальному, нужно разделить его на ±√(A²+B²).

Знак ± выбирается так, чтобы свободный член был отрицательным (ведь -p ≤ 0). Поэтому знак нормирующего множителя противоположен знаку Cен Вывод:

**Вывод:** Нормальное уравнение прямой x·cosα + y·sinα - p = 0 напрямую описывает ее через расстояние p от начала координат и угол α наклона этого перпендикуляра.
